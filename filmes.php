<?php
// --- CONFIGURAÇÃO E PERSISTÊNCIA ---
$arquivo = 'filmes.json';

// Inicializa o arquivo com dados padrão se ele não existir
if (!file_exists($arquivo)) {
    $dadosIniciais = [
        ['id' => 1, 'titulo' => 'Interestelar', 'genero' => 'Ficção Científica'],
        ['id' => 2, 'titulo' => 'O Poderoso Chefão', 'genero' => 'Crime'],
        ['id' => 3, 'titulo' => 'Matrix', 'genero' => 'Ação'],
        ['id' => 4, 'titulo' => 'Parasita', 'genero' => 'Drama'],
        ['id' => 5, 'titulo' => 'O Rei Leão', 'genero' => 'Animação']
    ];
    file_put_contents($arquivo, json_encode($dadosIniciais, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));
}

// Carrega os dados e garante que sempre teremos um array (evita erros se o arquivo estiver vazio)
$filmes = json_decode(file_get_contents($arquivo), true) ?? [];

// --- LÓGICA DE OPERAÇÕES (CONTROLLER) ---

// 1. ADICIONAR OU ATUALIZAR
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['salvar'])) {
    $id = $_POST['id'];
    // Salva o dado puro (sem htmlspecialchars aqui) para manter a integridade do JSON
    $titulo = trim($_POST['titulo']);
    $genero = trim($_POST['genero']);

    if (!empty($id)) {
        // Editar: mapeia o array e substitui o item correspondente
        foreach ($filmes as &$f) {
            if ($f['id'] == $id) {
                $f['titulo'] = $titulo;
                $f['genero'] = $genero;
            }
        }
    } else {
        // Adicionar: gera novo ID incrementando o maior ID atual
        $novoId = count($filmes) > 0 ? max(array_column($filmes, 'id')) + 1 : 1;
        $filmes[] = ['id' => $novoId, 'titulo' => $titulo, 'genero' => $genero];
    }

    file_put_contents($arquivo, json_encode(array_values($filmes), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));
    header("Location: " . $_SERVER['PHP_SELF']);
    exit;
}

// 2. EXCLUIR
if (isset($_GET['excluir'])) {
    $idExcluir = $_GET['excluir'];
    $filmes = array_filter($filmes, fn($f) => $f['id'] != $idExcluir);
    
    file_put_contents($arquivo, json_encode(array_values($filmes), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));
    header("Location: " . $_SERVER['PHP_SELF']);
    exit;
}

// 3. PREPARAR EDIÇÃO (Busca os dados para preencher o form)
$editFilme = null;
if (isset($_GET['editar'])) {
    $idEditar = $_GET['editar'];
    foreach ($filmes as $f) {
        if ($f['id'] == $idEditar) {
            $editFilme = $f;
            break;
        }
    }
}
?>

<!DOCTYPE html>
<html lang="pt-br">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>CRUD Filmes - Arquivo Único</title>
    <style>
        body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background: #f0f2f5; padding: 20px; color: #333; }
        .container { max-width: 800px; margin: auto; background: white; padding: 25px; border-radius: 10px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); }
        h2 { border-bottom: 2px solid #007bff; padding-bottom: 10px; color: #007bff; }
        
        form { display: flex; gap: 10px; margin-bottom: 30px; flex-wrap: wrap; background: #f8f9fa; padding: 15px; border-radius: 5px; }
        input { padding: 10px; border: 1px solid #ddd; border-radius: 5px; flex: 1; min-width: 150px; }
        
        table { width: 100%; border-collapse: collapse; margin-top: 10px; }
        th, td { padding: 12px; text-align: left; border-bottom: 1px solid #eee; }
        th { background: #007bff; color: white; }
        
        .btn { padding: 8px 15px; text-decoration: none; border-radius: 5px; font-weight: bold; border: none; cursor: pointer; transition: 0.3s; in-line: block; }
        .btn-add { background: #28a745; color: white; }
        .btn-edit { background: #ffc107; color: #212529; }
        .btn-del { background: #dc3545; color: white; }
        .btn:hover { opacity: 0.8; }
        .cancel-link { color: #666; font-size: 14px; align-self: center; }
    </style>
</head>
<body>

<div class="container">
    <h2>🎬 Gerenciador de Filmes</h2>

    <!-- FORMULÁRIO -->
    <form method="POST">
        <input type="hidden" name="id" value="<?= $editFilme['id'] ?? '' ?>">
        <!-- htmlspecialchars aplicado aqui para proteger o input de aspas e scripts -->
        <input type="text" name="titulo" placeholder="Título do Filme" value="<?= htmlspecialchars($editFilme['titulo'] ?? '') ?>" required>
        <input type="text" name="genero" placeholder="Gênero" value="<?= htmlspecialchars($editFilme['genero'] ?? '') ?>" required>
        
        <button type="submit" name="salvar" class="btn btn-add">
            <?= $editFilme ? 'Atualizar' : 'Adicionar' ?>
        </button>

        <?php if ($editFilme): ?>
            <a href="<?= $_SERVER['PHP_SELF'] ?>" class="cancel-link">Cancelar</a>
        <?php endif; ?>
    </form>

    <!-- LISTAGEM -->
    <table>
        <thead>
            <tr>
                <th>ID</th>
                <th>Título</th>
                <th>Gênero</th>
                <th>Ações</th>
            </tr>
        </thead>
        <tbody>
            <?php if (empty($filmes)): ?>
                <tr><td colspan="4" style="text-align:center;">Nenhum filme cadastrado.</td></tr>
            <?php else: ?>
                <?php foreach ($filmes as $f): ?>
                <tr>
                    <td><strong>#<?= (int)$f['id'] ?></strong></td>
                    <!-- htmlspecialchars aplicado na exibição dos dados da tabela -->
                    <td><?= htmlspecialchars($f['titulo']) ?></td>
                    <td><?= htmlspecialchars($f['genero']) ?></td>
                    <td>
                        <a href="?editar=<?= $f['id'] ?>" class="btn btn-edit">Editar</a>
                        <a href="?excluir=<?= $f['id'] ?>" class="btn btn-del" onclick="return confirm('Excluir este filme?')">Excluir</a>
                    </td>
                </tr>
                <?php endforeach; ?>
            <?php endif; ?>
        </tbody>
    </table>
</div>

</body>
</html>